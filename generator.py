...
def singleKill(mat):
	for i in range(mat.dims[0]):
		for j in range(mat.dims[1]):
			if mat[i,j] == 1:
				continue 
			if mat.inBound((i + 1, j)) and mat.inBound((i - 1, j)) and mat.inBound((i, j + 1)) and mat.inBound((i, j - 1)):
...
# save dungeon in file:
with open("dungeon.txt", 'w') as file:
	file.write(str(dungeon.dims[0]) + " " + str(dungeon.dims[1]) + "\n")
	file.write(str(dungeon.array))
file.close()
